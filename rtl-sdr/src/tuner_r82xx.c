			 ...
			 int len)
{
	...
	return (memcmp(&priv->regs[r], val, len) == 0);
}
...
static int r82xx_set_pll(struct r82xx_priv *priv, uint32_t freq)
{
	...
	uint8_t data[5];
	uint8_t regs[7]; /* shadow copy of regs 0x10 to 0x16 */
	...
	if (priv->cfg->rafael_chip == CHIP_R828D ||
	    rtlsdr_check_dongle_model(priv->rtl_dev, "RTLSDRBlog", "Blog V4") ||
	...
	/* Batch regs 0x10 to 0x16 into a single write to minimise I2C transactions */
	 ...
	 * using fixed-point rounding (exact, no iterative error accumulation).
	 */
	...
	/* pw_sdm: disable fractional SDM when sdm == 0 */
	val = (sdm == 0) ? 0x08 : 0x00;
...
//		usleep_range(sleep_time, sleep_time + 1000);
		...
		if (!i) {
			/* Didn't lock — increase VCO current and retry */
...

{
	...
	loop_through = 0x01;	/* r5[7], lt on (mask 0x80 → bit7=0 = loop-through enabled; osmocom mainline) */
...
int r82xx_set_freq(struct r82xx_priv *priv, uint32_t freq)
{
	...
	/* Blog V4 / V4L: auto-upconvert HF by 28.8 MHz so the SDR software needs no offset */
	upconvert_freq = (is_rtlsdr_blog_v4 || is_rtlsdr_blog_v4l)
	...
	if (is_rtlsdr_blog_v4) {
		...
		/* HF tracking filter bypass — must be set regardless of band change */
		if (band == HF) {
			rc = r82xx_write_reg_mask(priv, 0x1a, 0x40, 0xc3);
			if (rc < 0) goto err;
			rc = r82xx_write_reg_mask(priv, 0x1b, 0x00, 0xff);
			if (rc < 0) goto err;
		}
		...
		/* V4L has HF + UHF bands only (no VHF band) */
		band = (freq <= MHZ(28.8)) ? HF : UHF;

		/* HF tracking filter bypass */
		if (band == HF) {
			rc = r82xx_write_reg_mask(priv, 0x1a, 0x40, 0xc3);
			if (rc < 0) goto err;
			rc = r82xx_write_reg_mask(priv, 0x1b, 0x00, 0xff);
		...
		if (band != priv->input) {
			...
			/* V4L HF: cable 1 via reg 0x05 bit 6; UHF: air_in */
			cable_1_in = (band == HF) ? 0x40 : 0x00;
			rc = r82xx_write_reg_mask(priv, 0x05, cable_1_in, 0x40);
			if (rc < 0)
				goto err;

			/* GPIO5: high = bypass upconverter path (UHF), low = through upconverter (HF) */
			rc = rtlsdr_set_bias_tee_gpio(priv->rtl_dev, 5, (band == HF) ? 0 : 1);
			if (rc < 0)
				goto err;
		}
	} else {
...
int r82xx_init(struct r82xx_priv *priv)
{
	...
	/* Initialize shadow registers to zero before writing initial values */
