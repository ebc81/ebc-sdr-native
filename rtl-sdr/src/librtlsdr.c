...
/* __EBCANDROID__: <android/log.h> used to be included here and never used, while all
 * 83 fprintf(stderr, ...) diagnostics of this library (tuner detection, PLL failures,
 * exact sample rate) went nowhere on Android -- there is no stderr sink. Befund 10 of
 * ANALYSE.md, resolved in Phase 1 by ebc_log.h below. */


#include "librtlsdr_internal.h"

/* __EBCANDROID__: routes this file's fprintf(stderr, ...) diagnostics into logcat.
 * Without it they are lost on Android -- Befund 10 of ANALYSE.md. */
#include "ebc_log.h"

/* __EBCANDROID__: struct rtlsdr_dev, rtlsdr_tuner_iface_t, enum rtlsdr_async_status,
 * FIR_LEN, EEPROM_SIZE and STR_OFFSET were defined here until Phase 1 and were moved
 * verbatim into librtlsdr_internal.h, so that android/librtlsdr_andro.c can reach them
 * without #including this file. See that header for the full reasoning. */
...
int rtlsdr_set_center_freq(rtlsdr_dev_t *dev, uint32_t freq)
{
	...
	* Only when direct_sampling_mode == 0, i.e. the user has not forced a mode.
...
int rtlsdr_set_offset_tuning(rtlsdr_dev_t *dev, int on)
{
	...
	/* No offset tuning on R820T/R828D. The RTL-SDR-Blog fork additionally calls
	* rtlsdr_set_bias_tee(dev, on) here, so that host software without a bias-tee
	* control can switch it via the offset-tuning button. Removed on purpose: a call
	* that answers "not supported" must not put 5 V on the antenna connector as a
	* side effect. This is osmocom behaviour, and what rtlsdr433/rtlsdrPager do.
	* Apps that want the bias tee call rtlsdr_set_bias_tee() directly.
	*/
	if ((dev->tuner_type == RTLSDR_TUNER_R820T) ||
	    (dev->tuner_type == RTLSDR_TUNER_R828D))
		return -2;
...
found:
	...
	* indeterminate. Deriving force_bt from that garbage can switch the bias tee on
	* and put 5 V on the antenna connector of a device whose EEPROM never asked for
	* it. Same hardening as in librtlsdr_andro.c.
	*/
	memset(buf, 0, sizeof(buf));
	r = rtlsdr_read_eeprom(dev, buf, 0, EEPROM_SIZE);
	if (r < 0) {
		fprintf(stderr, "EEPROM read failed (%d); the Bias-T EEPROM hack is skipped\n", r);
		dev->force_bt = 0;
	} else {
...
int get_string_descriptor(int pos, uint8_t *data, char *str)
{
	...
	/* data always points at a 256-byte EEPROM image. Bound both the start offset
	 * and the length byte: a corrupt or unread EEPROM otherwise walks this read
	 * far past the caller's stack buffer (len is attacker/garbage controlled). */
	...
	if (pos + 1 < EEPROM_SIZE && data[pos + 1] != 0x03)
