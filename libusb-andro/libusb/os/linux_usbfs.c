	...
	struct usbfs_urb *urb)
{
	 ...
	 * Reading tpriv->urbs unlocked (as this did until v1.3.20) leaves a TOCTOU window --
	 * another thread can free tpriv->urbs between the test and the urb_idx arithmetic
	 * below, which is exactly the use-after-free this guard exists to prevent.
	 * History: rtlsdr433 found the Android-16 URB bug first (May 2026) and
	 * RTL_SDR_AIS_Driver took the guard over; rtlsdrPager corrected the lock ordering
	 * (August 2026). This is the union of all three. */
